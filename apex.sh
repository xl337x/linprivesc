...
APEX_LOG=""
APEX_TEMPFILES=""
APEX_PERSIST_FILE=""        # set by pick_persist_file — survives cleanup
APEX_PERSIST_DIR=""         # parent dir of APEX_PERSIST_FILE
APEX_TEE_PID=""             # PID of the `tee` started by `exec > >(tee ...)`
                            # in main(). Tracked so cleanup_apex can kill it
                            # before its bare `wait` (otherwise wait hangs
                            # forever — tee runs until apex's stdout closes).

# ── Heartbeat / disconnection detection ─────────────────────────────────────
APEX_HEARTBEAT_PID=""       # PID of the backgrounded heartbeat printer
APEX_ORIGIN_LOST=0          # set to 1 by heartbeat after 3 consecutive probe fails
APEX_HEARTBEAT_INTERVAL=10  # seconds between heartbeat lines
APEX_QUIET=0                # set by --quiet to suppress heartbeats
...
# ─── pick_persist_file ──────────────────────────────────────────────────────
# Decide where to persist the rendered findings output across the run so the
# operator can review them after APEX_TMP is wiped on exit. Tries — in order —
# operator override, $HOME, /tmp, /var/tmp, /dev/shm, current pwd. First
# writable wins. Sets APEX_PERSIST_FILE + APEX_PERSIST_DIR. Returns 1 (and
# leaves both empty) if no writable dir found → caller falls back to screen.
# Honors APEX_NO_SAVE=1 to opt out entirely (stealth mode).
pick_persist_file() {
    [ "${APEX_NO_SAVE:-0}" = "1" ] && return 1
    [ "${APEX_STEALTH:-0}" = "1" ] && return 1   # stealth → no on-disk trace

    local ts d cand
    ts=$(date +%Y%m%d_%H%M%S 2>/dev/null || printf '%s' "$$")
    local name="apex_findings_${ts}.txt"

    # Build candidate list — operator override first, then ranked by privacy
    local cands=""
    [ -n "${APEX_SAVE_DIR:-}" ] && cands="${APEX_SAVE_DIR}
"
    cands="${cands}${HOME:-}
/tmp
/var/tmp
...
$(pwd 2>/dev/null)"
    ...
    while IFS= read -r d; do
        [ -z "$d" ] && continue
        [ -d "$d" ] || continue
        # Probe-write: must actually be able to create a file (handles chattr +i,
        # readonly mounts, full disks, ACL-denied writes that -w can't see).
        cand="${d}/${name}"
        if : >"$cand" 2>/dev/null; then
            APEX_PERSIST_FILE="$cand"
            APEX_PERSIST_DIR="$d"
            chmod 600 "$cand" 2>/dev/null
            return 0
        fi
    done <<EOF
$cands
...
# ─── _apex_origin_probe ─────────────────────────────────────────────────────
# Quick TCP-connect test against APEX_ORIGIN_BASE host:port. Returns 0 if
# reachable, 1 if not. Wrapped in safe_run so a hung kernel TCP retry can't
# itself become a hang. Returns 0 silently in local-only mode (no origin) —
# heartbeat treats "no origin to probe" as "always ok".
_apex_origin_probe() {
    [ -z "${APEX_ORIGIN_BASE:-}" ] && return 0   # local-only run = no probe
    local strip host port
    strip="${APEX_ORIGIN_BASE#*://}"
    host="${strip%%/*}"
    case "$host" in
        *:*) port="${host##*:}"; host="${host%%:*}" ;;
        *)   port=80 ;;
    esac
    [ -z "$host" ] && return 0   # malformed — don't false-alarm
    # 2s budget. safe_run kills the probe if /dev/tcp hangs on kernel-level retry.
    safe_run 2 bash -c "exec 9<>/dev/tcp/${host}/${port} && exec 9<&-" >/dev/null 2>&1
}

# ─── _apex_heartbeat_start ──────────────────────────────────────────────────
# Background process that prints a status line every APEX_HEARTBEAT_INTERVAL
# seconds to stderr. Detects lost origin by probing every 30s; sets a flag
# file so the main process can short-circuit doomed staged fetches. Silenced
# under --quiet, --stealth, --test, or when stderr isn't a terminal-like fd.
_apex_heartbeat_start() {
    [ "${APEX_QUIET:-0}"   = "1" ] && return 0
    [ "${APEX_STEALTH:-0}" = "1" ] && return 0
    [ "${APEX_MODE:-}"     = "test" ] && return 0
    [ -n "$APEX_HEARTBEAT_PID" ] && return 0   # already running — idempotent

    # Snapshot needed values into the subshell environment
    local _parent_pid=$$
    local _origin_base="${APEX_ORIGIN_BASE:-}"
    local _interval="$APEX_HEARTBEAT_INTERVAL"
    local _tmp="${APEX_TMP:-/tmp}"
    local _stop_flag="${_tmp}/.apex_heartbeat_stop"

    rm -f "$_stop_flag" 2>/dev/null

    # Double-fork: outer subshell is synchronous and exits immediately; inner
    # subshell is backgrounded and reparented to init. This is critical —
    # otherwise bare `wait` in layer_1_dac / run_credential_hunt / layer_6
    # blocks forever waiting on the heartbeat loop. disown alone is NOT enough
    # because bash still tracks disowned PIDs for `wait` (only hides them from
    # `jobs`).
    (
        (
            # Clear inherited traps so parent EXIT doesn't re-fire here.
            trap - EXIT INT TERM HUP PIPE
            local _elapsed=0
            local _fails=0
            local _origin_state="ok"
            local _probe_due=30   # first probe at t=30s, then every 30s
            while :; do
                sleep "$_interval" 2>/dev/null || break
                _elapsed=$((_elapsed + _interval))

                # Stop conditions (in order: flag, parent dead, sleep failure)
                [ -e "$_stop_flag" ] && break
                if ! kill -0 "$_parent_pid" 2>/dev/null; then
                    break
                fi

                # Origin probe every 30s (skipped when no origin set)
                if [ -n "$_origin_base" ] && [ "$_elapsed" -ge "$_probe_due" ]; then
                    _probe_due=$((_probe_due + 30))
                    if _apex_origin_probe; then
                        _fails=0
                        _origin_state="ok"
                    else
                        _fails=$((_fails + 1))
                        _origin_state="FAIL(${_fails}/3)"
                        if [ "$_fails" -ge 3 ]; then
                            printf '\n[!] APEX ORIGIN UNREACHABLE (3 probes failed) — Kali listener down? VPN dropped? Continuing local-only enum…\n' >&2
                            : > "${_tmp}/.origin_lost" 2>/dev/null
                            _origin_base=""
                            _origin_state="LOST"
                        fi
                    fi
                ...
                if [ -n "$_origin_base" ] || [ "$_origin_state" = "LOST" ]; then
                    printf '[heartbeat] %ds elapsed — origin: %s\n' "$_elapsed" "$_origin_state" >&2
                else
                    printf '[heartbeat] %ds elapsed (local-only mode)\n' "$_elapsed" >&2
                fi
            done
        ) &
        # outer subshell exits here — inner reparented to init
    )
    APEX_HEARTBEAT_PID=1   # sentinel: running. Real PID is init's child now.
    return 0
}

# ─── _apex_heartbeat_stop ───────────────────────────────────────────────────
# Idempotent. Safe to call from traps. Sets a stop flag the heartbeat polls
# after each sleep — no `kill` needed because the heartbeat is no longer our
# child (double-forked to init), so we couldn't kill it from our session
# anyway without searching for it. The flag wins on the next loop iteration
# (≤ APEX_HEARTBEAT_INTERVAL seconds), and the kill-parent check is a safety
# net if the flag write fails.
_apex_heartbeat_stop() {
    [ -n "$APEX_HEARTBEAT_PID" ] || return 0
    : > "${APEX_TMP:-/tmp}/.apex_heartbeat_stop" 2>/dev/null
    APEX_HEARTBEAT_PID=""
    return 0
}

# ─── _apex_origin_is_lost ───────────────────────────────────────────────────
# Cheap check for code paths that may want to skip staged fetches early.
_apex_origin_is_lost() {
    [ "$APEX_ORIGIN_LOST" = "1" ] && return 0
    [ -n "$APEX_TMP" ] && [ -f "${APEX_TMP}/.origin_lost" ] && {
        APEX_ORIGIN_LOST=1
        return 0
    }
    return 1
...
cleanup_apex() {
    # Stop heartbeat first — it was double-forked to init, so jobs -p won't
    # catch it; the stop flag is what tells it to exit on its next tick.
    _apex_heartbeat_stop 2>/dev/null
    ...
    # Kill the persist `tee` explicitly. It is a child of apex (via
    # `exec > >(tee ...)`) but does NOT appear in `jobs -p`, and bare `wait`
    # below would block on it forever because tee only exits when apex's
    # stdout closes — which can't happen until cleanup completes.
    [ -n "$APEX_TEE_PID" ] && kill "$APEX_TEE_PID" 2>/dev/null && APEX_TEE_PID=""
...
apex_pspy_bg_wait_and_parse() {
    ...
    # on a corrupted pspy. Print a countdown every 10s so operator doesn't
    # think APEX hung. Silenced under --quiet.
    if [ "${APEX_QUIET:-0}" != "1" ] && [ "${APEX_STEALTH:-0}" != "1" ]; then
        local _waited=0
        while kill -0 "$APEX_PSPY_BG_PID" 2>/dev/null && [ "$_waited" -lt 80 ]; do
            sleep 10 2>/dev/null
            _waited=$((_waited + 10))
            kill -0 "$APEX_PSPY_BG_PID" 2>/dev/null && \
                printf '[APEX] pspy: %ds/70s — capturing background processes…\n' "$_waited"
        done
    fi
...
run_credential_hunt() {
    ...
    # IMPORTANT: explicit PIDs, NOT bare `wait`. Bare wait would also block on
    # the persist `tee` (started via `exec > >(tee ...)` in main) which runs
    # until apex itself exits — causing infinite hang.
    local _pids=()
    scan_ssh_artifacts                  & _pids+=("$!")
    scan_history_files                  & _pids+=("$!")
    scan_process_environ                & _pids+=("$!")
    scan_cloud_credentials              & _pids+=("$!")
    scan_container_secrets              & _pids+=("$!")
    scan_database_credentials           & _pids+=("$!")
    scan_git_credentials                & _pids+=("$!")
    scan_password_managers              & _pids+=("$!")
    scan_network_app_credentials        & _pids+=("$!")
    scan_ssl_tls_material               & _pids+=("$!")
    scan_hot_files                      & _pids+=("$!")
    scan_password_files                 & _pids+=("$!")
    check_authorized_keys_injectable    & _pids+=("$!")
    scan_bash_history                   & _pids+=("$!")
    scan_unix_sockets                   & _pids+=("$!")
    local _p
...
# ─── _layer_wrap_start ──────────────────────────────────────────────────────
# Print a one-line "RUNNING" banner BEFORE a layer's work begins so the
# operator never stares at a frozen screen during long parallel scans.
# Silenced under --quiet. Always safe to call — no side effects beyond print.
_layer_wrap_start() {
    [ "${APEX_QUIET:-0}" = "1" ] && return 0
    local num="${1:-?}" name="${2:-?}"
    name=$(safe_output "$name" 2>/dev/null || printf '%s' "$name")
    printf '\n[APEX] Layer %s (%s) — RUNNING…\n' "$num" "$name"
    return 0
}

layer_1_dac() {
    _layer_wrap_start 1 "DAC Graph"
    local _s _n
    _s=$(_finding_count)
    check_passwd_writable
    check_shadow_writable
    check_global_env_files
    # Explicit PID list — bare `wait` would also block on the persist `tee`
    # child (process substitution from main()), causing infinite hang.
    local _pids=()
    map_sudo                  & _pids+=("$!")
    map_suid_sgid             & _pids+=("$!")
    map_capabilities          & _pids+=("$!")
    map_cron                  & _pids+=("$!")
    map_systemd               & _pids+=("$!")
    map_write_surface         & _pids+=("$!")
    map_groups                & _pids+=("$!")
    map_groups_files          & _pids+=("$!")
    map_custom_binaries       & _pids+=("$!")
    map_nfs                   & _pids+=("$!")
    map_processes             & _pids+=("$!")
    map_neighbors_unreadables & _pids+=("$!")
    map_logrotate             & _pids+=("$!")
    map_motd                  & _pids+=("$!")
    local _p
    for _p in "${_pids[@]}"; do wait "$_p" 2>/dev/null; done
...
layer_2_deep_read() {
    _layer_wrap_start 2 "Deep Reader"
...
layer_3_credentials() {
    _layer_wrap_start 3 "Credentials"
    local _s _n
    _s=$(_finding_count)
    run_credential_hunt
    _n=$(( $(_finding_count) - _s ))
...
layer_6_dynamic() {
    _layer_wrap_start 6 "Dynamic (pspy)"
    local _s _n
    _s=$(_finding_count)
    if _layer_should_skip; then
    ...
    # Track explicit PIDs — bare `wait` would block on the persist `tee`.
    local _l6_pids=()
    ...
    # Reap only our own background children (NOT bare wait — would hang on tee).
    local _p
    for _p in "${_l6_pids[@]}"; do wait "$_p" 2>/dev/null; done
...
layer_7_kernel_cve() {
    _layer_wrap_start 7 "Kernel CVE"
...
layer_8_container() {
    _layer_wrap_start 8 "Container Escape"
    local _s _n
    _s=$(_finding_count)
    case "${IS_CONTAINER:-0}" in
        1|yes|true) ;;
...
layer_9_mac() {
    _layer_wrap_start 9 "MAC (AppArmor/SELinux)"
    local _s _n
    _s=$(_finding_count)
    if _layer_should_skip; then
...
layer_10_manual() {
    _layer_wrap_start 10 "Manual / Contextual"
...
# SIGHUP — controlling terminal died (SSH disconnect, parent shell killed).
# We still want the persist file complete on disk for post-mortem review.
# Don't try to write to stdout — it points at a dead pty. Exit 129 (128+1).
trap_hangup() {
    _apex_heartbeat_stop
    sync 2>/dev/null
    cleanup_apex
...
# Install all four. Single quotes so $? evaluation happens at trap-fire time,
# not at install time.
install_traps() {
    trap 'trap_exit' EXIT
    trap 'trap_interrupt' INT
    trap 'trap_terminate' TERM
    trap 'trap_hangup' HUP
    return 0
}
...
parse_args() {
    ...
    while [ "$#" -gt 0 ]; do
        case "$1" in
            ...
            --quiet|-q)    APEX_QUIET=1 ;;
            --no-save)     APEX_NO_SAVE=1 ;;
            --help|-h)     usage; exit 0 ;;
...
main() {
    ...
    # ── Persistent findings file (survives cleanup_apex) ────────────────────
    # Skipped under --stealth, --test, or APEX_NO_SAVE=1. Tee stdout into it
    # so the operator can review every screen line after the run completes.
    if [ "$APEX_MODE" != "test" ] && pick_persist_file; then
        # bash process substitution — guarded by detect_bash_features
        exec > >(tee -a "$APEX_PERSIST_FILE") 2>&1
        APEX_TEE_PID=$!   # bash 4+ exposes the proc-sub PID via $!
    fi

    # ── Heartbeat (after persist, before any long-running layer) ────────────
    # Backgrounded — prints "[heartbeat] Ns elapsed — origin: ok" every 10s
    # so the operator never stares at a frozen screen. Probes Kali origin
    # every 30s and flags APEX_ORIGIN_LOST after 3 fails. Self-silenced under
    # --quiet / --stealth / --test.
    _apex_heartbeat_start
    ...
    # ── Reasoner + output ───────────────────────────────────────────────────
    # Stop heartbeat before the rendered output so ranked PATH blocks aren't
    # interrupted by status lines mid-render.
    _apex_heartbeat_stop
    ...
    # ── Persist notice (or screen-only fallback) ────────────────────────────
    printf '\n'
    printf -- '------------------------------------------------------------------------\n'
    if [ -n "$APEX_PERSIST_FILE" ] && [ -s "$APEX_PERSIST_FILE" ]; then
        printf '  FINDINGS SAVED: %s\n' "$APEX_PERSIST_FILE"
        printf '  Review with:    less -R %s   (or)   grep PATH %s\n' \
            "$APEX_PERSIST_FILE" "$APEX_PERSIST_FILE"
        printf '  Size:           %s bytes\n' "$(wc -c <"$APEX_PERSIST_FILE" 2>/dev/null | tr -d ' ')"
    else
        printf '  NO WRITABLE PERSIST DIR FOUND — findings shown above are SCREEN-ONLY.\n'
        printf '  To save them, re-run and redirect:  bash apex.sh | tee /somewhere/out.txt\n'
        printf '  (Override search with: APEX_SAVE_DIR=/your/dir bash apex.sh)\n'
    fi
    printf -- '------------------------------------------------------------------------\n'
